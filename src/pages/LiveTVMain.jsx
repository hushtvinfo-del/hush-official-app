  ...
  return (
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <Button variant="ghost" onClick={() => navigate(createPageUrl(`MainMenu?playlistId=${playlistId}`))} className="mb-6 text-orange-300 hover:text-white hover:bg-orange-500/20">
          <ArrowLeft className="w-4 h-4 mr-2" />
          Back to Main Menu
        </Button>
        <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Live TV</h1>
            <p className="text-orange-300">Select a main category to browse</p>
        </div>
                            ...
                            <Card className="bg-gray-800/50 backdrop-blur-xl border-orange-500/30 hover:border-orange-500/60 transition-all group overflow-hidden text-center h-full flex flex-col">
                                <CardContent className="p-8 flex flex-col items-center justify-center flex-grow">
                                    <motion.div whileHover={{ scale: 1.05 }} className="flex flex-col items-center">
                                        {typeof config.icon === 'function' ? config.icon() : <config.icon className="w-20 h-20 text-orange-400 group-hover:text-orange-300 transition-colors" />}
